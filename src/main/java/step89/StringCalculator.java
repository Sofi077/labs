...
public class StringCalculator {
    public int add(String numbers) { //публічний нестатичний метод
        ...
        if (!numbers.isEmpty()){
            ...
            if (numbers.startsWith("//[")) {
                ...
                if (newline_index != -1) {
                    String delimiter_hand = numbers.substring(3, newline_index-1);  //встановлюємо роздільник
                ...
                int newline_index = numbers.indexOf('\n'); //позиція символу нового рядка
                if (newline_index != -1) {
                    String delimiter_hand = numbers.substring(2, newline_index);  //встановлюємо роздільник
                    delimiter_hand = Pattern.quote(delimiter_hand); //це щоб довгий роздільник не сприймався як спеціалні символи (регулярний літерал)
                    System.out.println("user-entered delimetr: "+delimiter_hand);
                    delimiter = "[" + delimiter_hand + ",\\n]";   //тепер всі роздільники (множинний роздільник)
                    numbers = numbers.substring(newline_index + 1);   //обрізаємо частину рядка це роздільник
                }
            ...
            try{
                if (negative_numb.size()==0) {
                    ...
                    throw new step89.NegativeNumberException("negativ number are not allowed");  //новий об'єкт винятку з повідомленням
                }
            } catch (step89.NegativeNumberException e)  //задаємо тип винятку
            {
    ...
    public static void main(String[] args) {
        ...
        step89.StringCalculator obj_1 = new step89.StringCalculator(); // створюємо об’єкт obj_1
